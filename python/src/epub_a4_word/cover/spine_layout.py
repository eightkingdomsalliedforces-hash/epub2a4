...
def build_spine_slots(layout: CoverLayout, accent_color: str) -> tuple[SpineLayoutTier, tuple[SpineSlot, ...]]:
    ...
    if tier == "full":
        slots = (
            SpineSlot("spine-title-main", _slice(safe, 0.16, 0.42), 14.0, "title", font_weight=600),
            ...
            SpineSlot("spine-author", _slice(safe, 0.72, 0.13), 10.0, "author"),
            SpineSlot("spine-internal-code", _slice(safe, 0.86, 0.05), 4.5, "code"),
            SpineSlot("spine-publisher-name", _slice(safe, 0.91, 0.07), 10.0, "publisher", font_weight=500),
        )
    elif tier == "compact":
        slots = (
            SpineSlot("spine-title-main", _slice(safe, 0.10, 0.52), 12.0, "title", font_weight=600),
            ...
            SpineSlot("spine-author", _slice(safe, 0.72, 0.15), 9.0, "author"),
            SpineSlot("spine-publisher-name", _slice(safe, 0.89, 0.09), 9.0, "publisher", font_weight=500),
        )
    else:
        slots = (
