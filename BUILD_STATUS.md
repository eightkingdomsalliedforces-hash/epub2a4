...
- 主視窗依可用螢幕範圍縮放並置中，封面左右工具欄可獨立捲動。
- 封面候選選取提供即時視覺回饋，套用區固定顯示在搜尋結果上方。
- 預設快速搜尋在取得第一批有效結果後停止；「搜尋更多」保留完整來源查詢。
- 封底優先使用 EPUB 內建圖片，其次本機圖片；公開書目候選只有在使用者明確指定時才可當作封底。
...
13. 在 1366 × 768 或更小可用高度的螢幕確認主視窗、封面左右欄及底部操作都能存取。
14. 選擇候選圖片後確認按鈕立即顯示已選用途，且套用按鈕不需捲到頁面最底部才可見。
15. 確認快速搜尋可較早顯示候選；需要更多結果時再按「搜尋更多」。
16. 確認封底優先採用 EPUB 內建或本機圖片，公開書目圖片不會被默認宣稱為真正封底。
