...
@pytest.fixture
def front_back_epub_factory(tmp_path: Path):
    def build(*, generic_back: bool = False) -> Path:
        path = tmp_path / ("candidate-back.epub" if generic_back else "front-back.epub")
        ...
        <package xmlns='http://www.idpf.org/2007/opf' xmlns:dc='http://purl.org/dc/elements/1.1/' version='3.0'>
          <metadata><dc:title>前後封面測試</dc:title><dc:creator>作者</dc:creator></metadata>
          <manifest>
        ...
        with ZipFile(path, "w") as archive:
            ...
            archive.writestr("OEBPS/Images/front.png", _png_bytes(600, 900), compress_type=ZIP_DEFLATED)
            archive.writestr("OEBPS/Images/back.png", _png_bytes(600, 900), compress_type=ZIP_DEFLATED)
        return path
