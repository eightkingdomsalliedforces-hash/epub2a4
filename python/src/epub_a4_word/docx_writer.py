...
def _add_image_block(
    ...
    try:
        stream, width_px, height_px = _normalized_image(
            data,
            media_types.get(block.resource_path, ""),
            block.resource_path,
        )
    ...
    run = paragraph.add_run()
    run.add_picture(stream, width=Pt(width_pt), height=Pt(height_pt))
    return None
...
def _populate_cell(
    ...
    if grid_cols == 1:
        start_cm = settings.cell_outer_margin_cm
        end_cm = settings.cell_outer_margin_cm
    else:
        ...
        if isinstance(block, TextBlock):
            ...
            if warning:
                ...
                placeholder = TextBlock(
                    (TextRun(f"[圖片無法顯示：{block.alt_text or block.resource_path}]"),),
                    style="body",
                )
                _add_text_block(cell, placeholder, settings, first)
            first = False
    return warnings
...
def write_docx(
    ...
    *,
    ...
    assert settings.paper_height_cm is not None
    assert settings.grid_rows is not None
    assert settings.grid_cols is not None
